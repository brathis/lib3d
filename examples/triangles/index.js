...
const t3 = lib3d.addTriangle(
  new Lib3dTriangle(
    ...
    new Lib3dVertex(1, 1, 0),
    new Lib3dVertex(0, 1, -1),
    new Lib3dColor(0.95, 0.7, 0.55, 1.0)
  )
);
...
const lz = lib3d.addLine(
  new Lib3dLine(
    ...
    new Lib3dColor(0, 0, 1, 1)
  )
);
lib3d.setCameraParameters(new Lib3dCameraParameters(1.0, 1.0, 1.0, 100, 0.1));

function updateScene(t) {
  // Like with the triangles, the changes to the camera parameters are immediately reflected in the buffers.
  lib3d.setCameraPosition(
    new Lib3dVertex(0.5 - 3 * Math.sin(t), 0.75, -0.5 - 3 * Math.cos(t))
  );
  lib3d.setCameraOrientation(
    new Lib3dCameraOrientation(
      new Lib3dVertex(-Math.cos(t), 0, Math.sin(t)),
      new Lib3dVertex(0, 1, 0),
      new Lib3dVertex(-Math.sin(t), 0, -Math.cos(t))
    )
  );
}
